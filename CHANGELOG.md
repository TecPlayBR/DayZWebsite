...
- **O instalador chamava o Webhook Secret do Mercado Pago de "opcional".** Não é. Em produção,
  sem ele, o site **recusa toda notificação de pagamento** e não existe nada que recupere depois
  — o jogador paga e a moeda nunca cai. Sem Access Token nenhum é mais seguro que Access Token
  sem Secret, porque aí o checkout fica em modo demonstração e nenhum dinheiro se move.
...
> Sem migration. Fix de código no webhook + reconciliação de dados na instalação afetada.
...
- **Removido vestígio de instalação real no template:** a página "Como Conectar" trazia um ID de BattleMetrics e uma porta de servidor hardcoded → agora `SEU-ID-BATTLEMETRICS` e porta padrão `2302`.
...
- Base pro resgate in-game (menu tecla I, escolher item/todos) que o mod vai implementar: o contrato do endpoint está documentado junto com a API de entrega.
